    ...
    useEffect(() => {
        ...
        Promise.all([apiFetch("/agents"), apiFetch("/areas")])
            ...
            .then(([agentsPayload, areasPayload]) => {
                if (!active) return;
                const agents = (agentsPayload.agents || []).filter((agent) => (
                    agent.status === "Actif"
                    && ["ORG_AGENT", "OPERATOR"].includes(agent.rawRole)
                ));
